...
import numpy as np
from GA import soga, calc_denorm

def optimizeGA(model):
    ...
    # Optimization - Trial
    result = soga("transonic_airfoil", model, nvar, ncon, lb, ub, npop, maxgen, pcross, pmut, cht_type=cht_types)
    best_objval = result[0]
    best_individual = result[1]
    best_individual = calc_denorm(best_individual[len(best_objval)-1],lb,ub)
    best_conval = result[2]



    filepath ='.\\Solutions\\summary_iter.txt'

    with open(filepath, "a") as f:
        f.write('optimized cd : ' + str(best_objval[len(best_objval)-1]) + '\n')
        f.write('DV : ' + str(best_individual) + '\n')
        f.write('const : ' + str(best_conval[len(best_objval)-1]) + '\n\n' )
